...
def test_correct_mislabeled_examples():
    ...
    expected_data_frame = pd.read_csv(
        Path("tests/dataset/sample_keypoints_38_corrected.csv")
    )
...
def test_drop_empty_images():
    ...
    expected_data_frame = pd.DataFrame.from_dict(
        sample_data_frames.drop_empty_images_after
    )
    pd.testing.assert_frame_equal(processed_df, expected_data_frame)
